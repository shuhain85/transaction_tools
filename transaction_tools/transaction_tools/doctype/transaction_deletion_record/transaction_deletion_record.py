...
class TransactionDeletionRecord(Document):
	...
	def populate_doctypes_to_be_ignored_table(self):
		...
		for doctype in doctypes_to_be_ignored_list:
			self.append("doctypes_to_be_ignored", {"reference_doctype": doctype})
	...
	def delete_company_transactions(self):
		...
		if not self.delete_transactions:
			...
			for docfield in self.doctypes:
				if docfield.reference_doctype != self.doctype and not docfield.done:
					no_of_docs = self.get_number_of_docs_linked_with_specified_company(
						docfield.reference_doctype, docfield.reference_name
					)
					if no_of_docs > 0:
						reference_docs = frappe.get_all(
							docfield.reference_doctype,
							filters={docfield.reference_name: self.company},
							limit=self.batch_size,
						)
						reference_doc_names = [r.name for r in reference_docs]

						self.delete_version_log(docfield.reference_doctype, reference_doc_names)
						self.delete_communications(docfield.reference_doctype, reference_doc_names)
						self.delete_comments(docfield.reference_doctype, reference_doc_names)
						self.unlink_attachments(docfield.reference_doctype, reference_doc_names)
						self.delete_child_tables(docfield.reference_doctype, reference_doc_names)
						self.delete_docs_linked_with_specified_company(
							docfield.reference_doctype, reference_doc_names
						)
						...
						# reset naming series
						naming_series = frappe.db.get_value("DocType", docfield.reference_doctype, "autoname")
						if naming_series:
							if "#" in naming_series:
								self.update_naming_series(naming_series, docfield.reference_doctype)
	...
	def populate_doctypes_table(self, tables, doctype, fieldname, no_of_docs):
		...
		if doctype not in tables:
			self.append(
				"doctypes", {"reference_doctype": doctype, "reference_name": fieldname, "no_of_docs": no_of_docs}
			)
	...
	def update_naming_series(self, naming_series, reference_doctype):
		if "." in naming_series:
			prefix, hashes = naming_series.rsplit(".", 1)
		else:
			prefix, hashes = naming_series.rsplit("{", 1)
		last = frappe.db.sql(
			f"""select max(name) from `tab{reference_doctype}`
						where name like %s""",
			prefix + "%",
		)
